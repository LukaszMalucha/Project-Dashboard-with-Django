...
from core.permissions import IsAdminOrReadOnly, IsOwnerOrReadOnly, IsFixingOrReadOnly
from core.permissions import IsAdminOrReadAndCreateOnly, IsAssignedAlready
...
class ProjectModelViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """Start new project"""
        profile = get_object_or_404(models.MyProfile, owner=self.request.user)
...
class TeamJoinView(views.APIView):
    ...
    def post(self, request, pk):
        ...
        if serializer.is_valid():
            ...
            team_member = models_project.TeamMembershipModel.objects.filter(project=project, member=request_user).first()
            if team_member:
                return Response({"error": "You have already joined that Team."},
...
class IssueAssignView(views.APIView):
    ...
    authentication_classes = (authentication.TokenAuthentication, authentication.SessionAuthentication)
    permission_classes = (permissions.IsAuthenticated, IsAssignedAlready)
...
class IssueFixedView(views.APIView):
    ...
    def post(self, request, id):
        ...
        return Response({"message": "Issue has been fixed."}, status=status.HTTP_200_OK)
