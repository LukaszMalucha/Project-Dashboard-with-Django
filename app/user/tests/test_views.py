...
ISSUE_FIXED_URL = reverse("user:issue-fixed", kwargs={'pk': 1})
...
class GamificationViewTests(TestCase):
    ...
    def test_with_personality_already_set(self):
        self.profile.personality = "tester"
        self.profile.save()
        response = self.client.get(GAMIFICATION_TEST_URL)
        self.assertEqual(response.status_code, 302)


class IssueFixedTests(TestCase):
    ...
    def setUp(self):
        ...
        self.profile = get_object_or_404(MyProfile, owner=self.user)
        self.project = ProjectModel.objects.create(name="Test", description="test",
                                                   proposed_by=self.user)
        self.issue = IssueModel.objects.create(name="Test", description="test", cost=12, project=self.project,
                                               assigned_to=self.user)

    def test_fixing_issue(self):
        self.client.force_login(self.user)
        response = self.client.get(ISSUE_FIXED_URL)
        self.assertEqual(response.status_code, 302)
        exist = IssueModel.objects.filter(
            name="Test",
        ).exists()
        self.assertFalse(exist)

    def test_access_issue_not_assigned(self):
